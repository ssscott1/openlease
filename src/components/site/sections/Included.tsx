...
const ICONS: Record<string, React.ReactNode> = {
  ...
  tyres: (
    <>
      ...
      <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm-3-8v3m0 10v3m8-8h-3M7 12H4" />
    </>
  ),
  rego: (
...
export async function Included({ items }: { items: string[] }) {
  const t = await getTranslations("included");

  return (
    <section id="included" className="scroll-mt-20 bg-mist py-16 sm:py-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <ul className="mt-12 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
            ...
            >
              <span className="inline-flex h-10 w-10 items-center justify-center rounded-lg bg-accent-soft text-accent-strong">
                <svg aria-hidden className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor">
                  {ICONS[key] ?? ICONS.maintenance}
                </svg>
              </span>
              <span className="text-sm font-medium">
                {t.has(`items.${key}`) ? t(`items.${key}`) : key}
              </span>
            </li>
          ))}
          <li className="flex flex-col items-start gap-3 rounded-xl border-2 border-dashed border-line bg-transparent p-5">
            ...
            <span className="text-xs text-ink-soft">{t("fuelExcludedNote")}</span>
          </li>
        </ul>
      </div>
    </section>
